...
def establecer_dificultad(datos_juego: dict, dificultad: str) -> bool:
...
def verificar_respuesta(pregunta_actual:dict, datos_juego:dict, respuesta:int):
    """
    Verifica si la respuesta es correcta y actualiza puntuación, vidas y estadísticas
    """
    if not isinstance(pregunta_actual, dict):
        return None
    ...
    from preguntas import actualizar_estadistica, guardar_preguntas, lista_preguntas
    
    acierto = actualizar_estadistica(pregunta_actual, respuesta)
...
def wrap_text(text, font, max_width):
    """
    SUMMARY:
        Divide un texto largo en varias líneas para que ninguna supere
        un ancho máximo en píxeles. Devuelve una lista con cada línea
        ya ajustada.

    ARGS:
        text (str): Texto completo a dividir.
        font (pygame.font.Font): Fuente usada para medir el tamaño del texto.
        max_width (int): Ancho máximo permitido para cada línea (en píxeles).

    RETURNS:
        list[str]: Lista de líneas, cada una ajustada para no superar max_width.
    """
    ...
    # Texto que se va armando palabra por palabra
    actual_line = ""
    ...
    for word in words:
        ...
        # Si el ancho de la línea sigue siendo menor o igual al máximo permitido...
        if font.size(test_line)[0] <= max_width:
            actual_line = test_line  # ...actualiza la línea actual.
        else:
            # Si la línea actual ya tiene contenido, se guarda como línea final
            if actual_line:
    ...
    # Agrega la última línea que haya quedado sin guardar
    if actual_line:
        lines.append(actual_line)

    # Devuelve todas las líneas ajustadas
    return lines